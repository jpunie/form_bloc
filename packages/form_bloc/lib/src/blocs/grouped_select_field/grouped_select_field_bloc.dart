...
/// from multiple items.
class GroupedSelectFieldBloc<Group, Value, ExtraData> extends SingleFieldBloc<
    Value?,
    Value,
    GroupedSelectFieldBlocState<Group, Value, ExtraData>,
    ExtraData?> {
  ...
  GroupedSelectFieldBloc({
    ...
    Map<Group, List<Value>> items = const {},
    ...
    ExtraData? extraData,
          ...
          initialState: GroupedSelectFieldBlocState(
  ...
  void updateItems(Map<Group, List<Value>> items) {
    emit(state.copyWith(
      items: items,
      value: items.values.contains(value) ? null : Param(null),
    ));
  }
}
